...
public class Servicos extends Service implements LocationListener {
	...
	public void onLocationChanged(Location local) {
		...
		Log.i("Local", coordenadas.size()+" - "+local.getLatitude()+"/"+local.getLongitude()+"  "+new Timestamp(System.currentTimeMillis()));
		if(coordenadas.size() == tempoAtu){
			ConnectWebService connectWebService = new ConnectWebService();
			connectWebService.execute(coordenadas.get(coordenadas.size()-1).getLatitude()+"",coordenadas.get(coordenadas.size()-1).getLongitude()+"", 
					coordenadas.get(coordenadas.size()-1).getSpeed()+"",imei);
